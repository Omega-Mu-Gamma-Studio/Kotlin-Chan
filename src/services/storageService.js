...
const localAdapter = {
  ...
  clear: () => {
    // Only clear kotlinchan-prefixed keys
    Object.keys(localStorage)
      .filter(k => k.startsWith('kotlinchan-'))
...
export const storageService = {
  ...
  getSettings: () => adapter.get('kotlinchan-settings') || {},
  saveSettings: (settings) => adapter.set('kotlinchan-settings', settings),

  // Cached lesson data (optional optimization)
  getCachedLesson: (lessonId) => adapter.get(`kotlinchan-lesson-${lessonId}`),
  cacheLesson: (lessonId, data) => adapter.set(`kotlinchan-lesson-${lessonId}`, data),
